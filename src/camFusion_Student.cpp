...
void clusterKptMatchesWithROI(BoundingBox &boundingBox, std::vector<cv::KeyPoint> &kptsPrev, std::vector<cv::KeyPoint> &kptsCurr, std::vector<cv::DMatch> &kptMatches)
{
    ...
    std::vector<double> distances{}, sorted_distances{};

    for (const auto &match : kptMatches)
    {
        if (boundingBox.roi.contains(kptsCurr[match.trainIdx].pt))
        {
            const cv::Point2d &curr_pt{kptsCurr[match.trainIdx].pt};
            const cv::Point2d &prev_pt{kptsPrev[match.queryIdx].pt};

            const double dx{curr_pt.x - prev_pt.x};
            const double dy{curr_pt.y - prev_pt.y};
    ...
    const double median_distance = sorted_distances[sorted_distances.size() / 2];

    const double margin{0.5};

    for (uint16_t idx{}; idx < distances.size(); ++idx)
    {
        if (distances[idx] < (1 + margin) * median_distance && distances[idx] > (1 - margin) * median_distance)
        {
            boundingBox.kptMatches.push_back(tmp_matches[idx]);
            boundingBox.keypoints.push_back(kptsCurr[tmp_matches[idx].trainIdx]);
        }
    }
}
...
void matchBoundingBoxes(std::vector<cv::DMatch> &matches, std::map<int, int> &bbBestMatches, DataFrame &prevFrame, DataFrame &currFrame)
{
    ...
    for (const auto &match : matches)
    {
        ...
        std::vector<int> curr_bb{};
        std::vector<int> prev_bb{};

        for (const auto &bb : currFrame.boundingBoxes)
            if (bb.roi.contains(curr_pt))
                curr_bb.push_back(bb.boxID);

        for (const auto &bb : prevFrame.boundingBoxes)
            if (bb.roi.contains(prev_pt))
                prev_bb.push_back(bb.boxID);
